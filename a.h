...
class EliteCultist: public Cultist
{
...
public:
    EliteCultist(int stage):Cultist(stage)
    {
        name="Elite Cultist";
        special="Gains 6 strength. Inflicts 1 Dazed.";
        Hp*=2;
        dmg*=1.5;
    }
...
class EliteAssasin: public Assasin
{
...
public:
    EliteAssasin(int stage):Assasin(stage)
    {
        type=0;
        name="Elite Assasin";
        special="Inflicts 2 Vulnerable. Inflicts 2 Weak.";
        Hp*=2;
        dmg*=2;
    }
};
...
int Combat(Player& p,int stage, PotionBelt& pot)
{
    if(stage%3==0)
    {
        if(stage%2==0)
        {
            EliteAssasin e=EliteAssasin(stage);///polimorfism exec
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Assasin::IncCounter();
        }
        else
        {
            EliteCultist e=EliteCultist(stage);///polimorfism exec
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Cultist::IncCounter();
        }
    }
    else
    {
        ...
        {
            Slaver e=Slaver(stage);
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Slaver::IncCounter();
        }
        else if(enemyvar==2)
        {
            Bruiser e=Bruiser(stage);
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Bruiser::IncCounter();
        }
        else if(enemyvar==3)
        {
            Rammer e=Rammer(stage);
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Rammer::IncCounter();
        }
        else if(enemyvar==4)
        {
            Cultist e=Cultist(stage);
            while(p.get_value("Hp")>0&&e.get_value("Hp")>0)
            Turn(p,e,pot);
            if(p.get_value("Hp")>0)Cultist::IncCounter();
        }
